...
def save_annotation_on_original_image(imgs,save_dir,filename,colormap_type):
  original_image=imgs[0]
  label=imgs[1]
  result=original_image+color_map_backup[label]
  pil_image = img.fromarray(result.astype(dtype=np.uint8))
  with tf.gfile.Open('%s/%s.png' % (save_dir, filename), mode='w') as f:
    pil_image.save(f, 'PNG')
